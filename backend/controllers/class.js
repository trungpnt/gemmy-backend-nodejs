...
function getEndDate(start_date, total_sessions, class_session) {
  ...
  total_sessions = 5;
  class_session = {
    0: { day_session: "evening", day: "6" },
    1: { day_session: "evening", day: "0" },
  }; //monday, thurday
  ...
  if (day_start === parseInt(class_session[0].day)) {
    date_remain_in_start_week = new Date(
      addDays(
        start_date,
        Number(class_session[1].day) - Number(class_session[0].day)
      )
    );
  ...
  let number_special_day = specialDayController.getSpecialDaysInTimeRange(
    start_date,
    end_date,
    class_session
  );

  let end_date_official = new Date(
    addDays(start_date, total_weeks * 7 + number_special_day)
  );
...
exports.createClass =  (req, res, next) => {
  ...
  const classModel = new Class({
    ...
    class_notes : req.body.class_notes
  });
  classModel
    .save()
    .then((createdClass) => {
...
exports.updateClass = (req, res, next) => {
  //Update a full class
  if (
    req.body.class_label != null &&
    req.body.class_level != null &&
    req.body.class_name
  ) {
    Class.findById(req.params.id)
      .then((class_found) => {
        if (class_found) {
          var class_code = genClassCode(
            req.body.class_level,
            req.body.class_label,
            req.body.class_name
          );
          ...
          Class.findOne({ class_code: class_code })
            .then((result) => {
              if (result) {
                res.status(200).json({
                  message: "The class_code with the same name already exists !",
                });
              } else {
  ...
  //UPDATE STUDENT => ADD NEW STUDENT TO THIS CLASS
    ...
    req.body.student_list != null &&
    req.body.is_active == null &&
    req.body.class_session == null
  ) {
    Class.findById(req.params.id)
      .then((class_found) => {
        if (class_found) {
...
exports.getClassTimeRange = (req, res, next) => {
  ...
  const ClassQuery = Class.find(
    { is_active: true, remaining_slots: { $gt: 1 } }.select({ student_list: 0 })
  );
